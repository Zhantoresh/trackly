...
def create_project(
    ...
    db.add(project)
    db.flush()  # чтобы получить project.id до commit

    # Автоматически добавляем создателя как owner в project_members
...
def get_my_projects(
    ...
    # Возвращаем все проекты где юзер является участником
